...
export class ConsumptionDetailsComponent implements OnInit {
    ...
    private addConsumption(consumption: Consumption): void {
        ...
        this.fsService.addElementToArrayF(`${this.baseUrl}/${this.store.course().id}`, `consumptions`, consumption)
            .then((res: any) => {
                ...
                // this.store.toggleConsumptionDetailsVisible(false);
                // this.store.courseSelected(this.store.loadCourses())
                this.form.reset();
            })
    ...
    private deleteConumption(consumption) {
        const path = `${this.baseUrl}/consumptions/${this.store.course().id}/details`
        return this.fsService.removeElementFromArray(path, 'consumptions', consumption)
